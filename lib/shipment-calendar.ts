...
export function shipmentDay(load:{scheduled_at:string;status?:string;shipped_at?:string|null}):string {
  return dayKey(load.status==="Concluído"&&load.shipped_at?load.shipped_at:load.scheduled_at);
}
