...
export default function Home() {
  ...
  return <main className={`shell ${tv?"tv":""}`}>
    ...
    {details&&<Details load={details} late={isLate(details,now)} photos={photos.filter(photo=>photo.shipment_id===details.id)} audits={audits.filter(audit=>audit.shipment_id===details.id)} onClose={()=>setDetails(null)} onPhoto={canWrite?uploadLoadPhoto:undefined} onEdit={canWrite?()=>{setModal({...details,scheduled_at:details.scheduled_at.slice(0,16)});setDetails(null)}:undefined}/>} 
...
function LoadCard({load,canWrite,isAdmin,onOpen,onEdit,onMove,onDelete,onPhoto,photos}:{load:Shipment;canWrite:boolean;isAdmin:boolean;onOpen:()=>void;onEdit:()=>void;onMove:(l:Shipment,d:number)=>void;onDelete:()=>void;onPhoto:(load:Shipment,file?:File)=>void;photos:ShipmentPhoto[]}){
