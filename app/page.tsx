...
function LoadCard({load,canWrite,isAdmin,onOpen,onEdit,onMove,onDelete,onPhotoView,photos}:{load:Shipment;canWrite:boolean;isAdmin:boolean;onOpen:()=>void;onEdit:()=>void;onMove:(l:Shipment,d:number)=>void;onDelete:()=>void;onPhotoView:()=>void;photos:ShipmentPhoto[]}){
 ...
 return <article className={'load status-'+STAGES.indexOf(load.status)}><button className="card-main" onClick={onOpen}><div className="load-heading"><div><h3>{load.number}</h3><p>{load.carrier}</p></div><span className="status-chip">{load.status}</span></div><div className="load-import-data"><span><small>PRAZO</small><b>{new Date(load.scheduled_at).toLocaleDateString("pt-BR")}</b></span><span><small>QUANTIDADE</small><b>{load.quantity.toLocaleString("pt-BR")}</b></span><span><small>STATUS DO PRAZO</small><b>{isLate(load,Date.now())?"Atrasado":"Dentro do prazo"}</b></span></div></button><div className="card-actions">{!done&&canWrite&&<button className="primary action-main" onClick={()=>onMove(load,1)}>{load.status==='Pendente'?'Iniciar':load.status==='Falta item'?'Retomar':'Concluir'}</button>}{load.status==='Em processo'&&canWrite&&<button className="primary" onClick={()=>onMove(load,-1)}>Falta item</button>}{photos[0]&&<button className="photo-preview card-photo" title="Ver fotos da carga" onClick={onPhotoView}><img src={photos[0].url} alt={`Foto da carga do lote ${load.number}`}/><b>{photos.length}</b></button>}{canWrite&&<button className="primary" onClick={onEdit}>Editar</button>}{isAdmin&&<button className="primary danger-action" onClick={onDelete}>Excluir</button>}</div></article>
}

function LoadModal({value,onChange,onClose,onSubmit}:{value:FormLoad;onChange:(v:FormLoad)=>void;onClose:()=>void;onSubmit:(e:FormEvent<HTMLFormElement>,photos?:File[])=>void}){
...
function PhotoViewer({load,photos,onClose}:{load:Shipment;photos:ShipmentPhoto[];onClose:()=>void}){return <div className="overlay" onMouseDown={e=>e.target===e.currentTarget&&onClose()}><section className="modal photo-viewer"><div className="modal-title"><div><small>FOTOS DA CARGA</small><h2>Lote {load.number}</h2></div><button onClick={onClose}><X/></button></div><div className="photo-gallery">{photos.length?<div>{photos.map(photo=><figure key={photo.id}><img src={photo.url} alt={`Foto da carga do lote ${load.number}`}/><figcaption>{new Date(photo.created_at).toLocaleString("pt-BR")}</figcaption></figure>)}</div>:<p>Nenhuma foto cadastrada.</p>}</div></section></div>}
