...
export default function Home() {
  ...
  return <main className={`shell ${tv?"tv":""}`}>
    ...
    {view==="board"&&<>
      ...
      <div className="operation-grid"><section className="board-wrap"><div className="section-title"><div><small>PROGRAMAÇÃO DE LOTES</small><h2>{stageFilter==="Todos"?"Todas as requisições":stageFilter}</h2></div><span>{filtered.length} registros</span></div>{filtered.length?<><div className="shipment-table-head" aria-hidden="true"><span>Lote / unidade</span><span>Data</span><span>Quantidade</span><span>Descrição</span><span>Situação</span><span>Ações</span></div><div className="request-grid">{filtered.slice((currentPage-1)*12,currentPage*12).map(load=><LoadCard key={load.id} load={load} canWrite={!!canWrite} onOpen={()=>setDetails(load)} onMove={move}/>)}</div></>:<div className="empty-state"><Package/><h3>Nenhuma requisição nesta aba</h3><p>Use outra aba ou cadastre uma nova requisição.</p></div>}</section><aside className="queue"><div className="section-title"><h2><Clock size={19}/> Prioridades</h2><span>fila</span></div>{queue.length?queue.map((load,i)=><button key={load.id} onClick={()=>setDetails(load)}><b>{String(i+1).padStart(2,"0")}</b><span><strong>{load.number} · {load.carrier}</strong><small>{load.status}</small></span></button>):<p className="empty">Nenhuma requisição na fila.</p>}</aside></div>
    </>}
...
function LoadCard({load,canWrite,onOpen,onMove}:{load:Shipment;canWrite:boolean;onOpen:()=>void;onMove:(l:Shipment,d:number)=>void}){
 ...
 {canWrite&&!done&&<div className="card-actions"><button className="primary action-main" onClick={()=>onMove(load,1)}>{load.status==='Pendente'?'Iniciar':load.status==='Falta item'?'Retomar':'Concluir'}<ArrowRight size={15}/></button></div>}
 </article>
}
