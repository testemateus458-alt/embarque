...
import {
  ...
  Camera, History, LogOut, Monitor, Package, Pencil, Plus, Search, Settings2,
...
export default function Home() {
  ...
  async function uploadLoadPhoto(load:Shipment,file?:File){
    ...
    try{
      ...
      const {error:updateError}=await supabase!.from("shipments").update({photo_url:urlData.publicUrl}).eq("id",load.id);
  ...
  return <main className={`shell ${tv?"tv":""}`}>
    ...
    <header><button className="brand" onClick={()=>setView("board")}><img src="/packem-mark.png" alt=""/><span>PACKEM<small>OPERAÇÕES</small></span></button><span className="live"><i/> {connection.toUpperCase()}</span><nav><button className={view==="dashboard"?"active":""} onClick={()=>setView("dashboard")}><Activity size={16}/> Dashboard</button><button className={view==="board"?"active":""} onClick={()=>setView("board")}>Operação</button><button className={view==="history"?"active":""} onClick={()=>setView("history")}><History size={16}/> Histórico</button>{profile?.role==="admin"&&<button className={view==="team"?"active":""} onClick={()=>setView("team")}><Users size={16}/> Equipe</button>}</nav>{!demoUnlocked&&<button className="unlock-button" onClick={()=>{setAccessError("");setUnlockOpen(true)}}>Liberar alterações</button>}<button onClick={async()=>{if(tv){await document.exitFullscreen?.();setTv(false);}else{try{await document.documentElement.requestFullscreen?.();setTv(true);}catch{setTv(false);}}}}><Monitor size={16}/> {tv?"Sair da TV":"Modo TV"}</button></header>
...
function LoadCard({load,canWrite,isAdmin,onOpen,onEdit,onMove,onDelete,onPhoto}:{load:Shipment;canWrite:boolean;isAdmin:boolean;onOpen:()=>void;onEdit:()=>void;onMove:(l:Shipment,d:number)=>void;onDelete:()=>void;onPhoto:(load:Shipment,file?:File)=>void}){
 ...
 <div className="load-import-data"><span><small>Data</small>{new Date(load.scheduled_at).toLocaleDateString('pt-BR')}</span><span><small>Quantidade</small>{Number(load.volumes).toLocaleString('pt-BR')}</span></div>{load.photo_url&&<button className="photo-preview" title="Ver foto da carga" onClick={onOpen}><img src={load.photo_url} alt={`Foto da carga do lote ${load.number}`}/></button>}
 <span className="row-status"><span className="status-chip">{load.status}</span></span>
 {canWrite&&<div className="card-actions"><label className="photo-action" title="Adicionar foto da carga"><Camera size={15}/><span>Foto</span><input type="file" accept="image/jpeg,image/png,image/webp" onChange={event=>{void onPhoto(load,event.target.files?.[0]);event.currentTarget.value=""}}/></label>{!done&&<button className="primary action-main" onClick={()=>onMove(load,1)}>{load.status==='Pendente'?'Iniciar':load.status==='Falta item'?'Retomar':'Concluir'}<ArrowRight size={15}/></button>}{load.status==='Em processo'&&<button className="shortage-action" title="Informar falta de item" onClick={()=>onMove(load,-1)}><AlertTriangle size={14}/></button>}<button className="icon-action" title="Editar lote" onClick={onEdit}><Pencil size={15}/><span>Editar</span></button>{isAdmin&&<button className="icon-action danger-action" title="Excluir lote" onClick={onDelete}><Trash2 size={15}/><span>Excluir</span></button>}</div>}
 </article>
}
...
function Details({load,onClose,onEdit}:{load:Shipment;late:boolean;onClose:()=>void;onEdit?:()=>void}){return <div className="overlay" onMouseDown={e=>e.target===e.currentTarget&&onClose()}><section className="modal details simple-details"><div className="modal-title"><div><small>LOTE</small><h2>{load.number}</h2></div><button onClick={onClose}><X/></button></div><dl><div><dt>Unidade</dt><dd>{load.carrier||"—"}</dd></div><div><dt>Situação</dt><dd>{load.status}</dd></div>{load.notes&&<div className="wide"><dt>Descrição</dt><dd>{load.notes}</dd></div>}</dl>{load.photo_url&&<div className="load-photo"><img src={load.photo_url} alt={`Foto da carga do lote ${load.number}`}/></div>}{load.status==="Falta item"&&load.missing_item_notes&&<div className="warning"><AlertTriangle/><span><b>Item faltante</b><br/>{load.missing_item_notes}</span></div>}<footer>{onEdit&&<button className="primary" onClick={onEdit}><Pencil size={16}/> Editar requisição</button>}</footer></section></div>}
